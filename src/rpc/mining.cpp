...
static RPCHelpMan submitNFT()
{
    ...
    return RPCHelpMan{
        ...
        "\n",
        {
            {"command", RPCArg::Type::STR, RPCArg::Optional::NO, "NFT command: add-class or add-asset"},
            ...
            {"nftclassid", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "hex-encoded asset class ID, or empty if creating asset class."},
        },
        {
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (gArgs.GetArg("-nftnode", "") == "true") {
                ...
                if (command == "add-class") {
                    ...
                    std::vector<unsigned char> vMetaData = ParseHex(nftdata.substr(4, nftdata.size() - 20).c_str()); //remove first 2 bytes of len and last 8 bytes of max count  (2 chars per hex)
                    ...
                    CNFTAssetClass* assetClass = g_nftMgr->retrieveAssetClassFromDatabase(nftAssetClassID);

                    if (assetClass != NULL) {
                        std::string assetHash = HexStr(nftID);
                        if (!g_nftMgr->assetInDatabase(HexStr(assetHash))) {
                            ...
                            if ((newAsset->serial >= 0) && (newAsset->serial < assetClass->maxCount)) {
                                if (!g_nftMgr->assetSerialExists(nftAssetClassID, newAsset->serial)) {
                                    ...
                                    result = "duplicate-serial";
                                    delete newAsset;
                                }
                            } else {
                                result = "invalid-serial";
                                delete newAsset;
                            }
                        } else {
                            result = "duplicate-hash";
                        }
                    } else {
                        result = "invalid-asset-class";
                    }
                } else {
                    result = "invalid-command";
                }
